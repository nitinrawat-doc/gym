...
export default function AddMember() {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try { await api.post('/members', form); navigate('/members'); }
  ...
  return (
    <div>
      <div style={{display:'flex',alignItems:'center',gap:'12px',marginBottom:'20px'}}>
        <button onClick={() => navigate('/members')} style={{background:'none',border:'none',cursor:'pointer',color:'rgba(255,255,255,0.4)',fontSize:'20px',padding:0,lineHeight:1}}>←</button>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <input type="text" value={form.name} onChange={e=>set('name',e.target.value)} placeholder="Arjun Kumar" required style={inputStyle}
        ...
        <input type="tel" value={form.phone} onChange={e=>set('phone',e.target.value)} placeholder="+91 98765 43210" required style={inputStyle}
          onFocus={e=>e.target.style.borderColor='rgba(0,255,135,0.4)'} onBlur={e=>e.target.style.borderColor='rgba(255,255,255,0.1)'}/>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'8px',marginBottom:'14px'}}>
          {PLANS.map(p => (
            <button type="button" key={p.value} onClick={() => set('plan', p.value)} style={{
              padding:'12px 6px', borderRadius:'14px', textAlign:'center', cursor:'pointer', transition:'all .15s',
        ...
        <input type="number" value={form.fee_amount} onChange={e=>set('fee_amount',e.target.value)} placeholder="1500" required min="1" style={inputStyle}
          onFocus={e=>e.target.style.borderColor='rgba(0,255,135,0.4)'} onBlur={e=>e.target.style.borderColor='rgba(255,255,255,0.1)'}/>

        <label style={{color:'rgba(255,255,255,0.45)',fontSize:'12px',display:'block',marginBottom:'6px'}}>Joining Date *</label>
        <input type="date" value={form.joining_date} onChange={e=>set('joining_date',e.target.value)} required style={{...inputStyle,marginBottom:'20px'}}
          onFocus={e=>e.target.style.borderColor='rgba(0,255,135,0.4)'} onBlur={e=>e.target.style.borderColor='rgba(255,255,255,0.1)'}/>
